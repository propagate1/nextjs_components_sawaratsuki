...
const MENU_ITEMS: { label: string; href: string; external?: boolean; ruby?: { base: string; rt: string } }[] = [
    { label: "機能", href: "javascript:void(0)" },
    { label: "事例", href: "javascript:void(0)" },
    { label: "料金", href: "javascript:void(0)" },
    { label: "無料オンライン相談", href: "https://team.liteview.jp/team/consulting" },
    { label: "リファラル採用", href: "https://refer.liteview.jp/", external: true, ruby: { base: "リファラル採用", rt: "LITEVIEW Refer" } },
    { label: "新規事業MVP", href: "https://frontier.liteview.jp/", external: true, ruby: { base: "新規事業MVP", rt: "LITEVIEW Frontier" } }
];
...
export default function Header4007() {
    ...
    return (
        <header id="ltv-corp-biz_header" className="w-full max-[1199px]:fixed max-[1199px]:top-0 max-[1199px]:left-0 bg-white z-[99999]">
            <div className="header__inner max-[1199px]:p-4 min-[1200px]:px-8 flex items-center justify-between gap-8">
                {/* 上部 左: ロゴ + メニュー */}
                <div className="header__top inline-block align-middle whitespace-nowrap min-[1200px]:font-[0] w-auto max-[1199px]:w-[40%]">
                    <a href="https://team.liteview.jp/team" aria-current="page" className="header__top-image inline-block align-middle pr-0 min-[1200px]:pr-8">
                        <Image
                            src="/img/logo.png" // liveteam/live_files/logo.png を public/img へコピー
                            alt="LITEVIEW"
                            width={130}
                            height={40}
                            className="min-[1200px]:w-[130px] max-[1199px]:w-[104px] min-[1200px]:-mt-4"
                            priority
                        />
                    </a>
                    {/* デスクトップメニュー */}
                    <nav className="header__top-menu horizontal-menu max-[1199px]:hidden inline-block align-middle whitespace-nowrap">
                        {MENU_ITEMS.map((item) => (
                            <a
                                ...
                                className="inline-block text-[14px] font-bold leading-[48px] pr-4 last:pr-0"
                            >
                                {item.ruby ? (
                                    <ruby>{item.ruby.base} <rt className="block text-[40%] -mb-4 pl-[2%] tracking-[5px]">{item.ruby.rt}</rt></ruby>
                                ) : (
                                    item.label
                                )}
                            </a>
                ...
                <div className="header__bottom inline-block align-middle text-right ml-auto whitespace-nowrap max-[1199px]:hidden">
                    <a
                        ...
                        className="inline-block font-bold text-white text-[14px] bg-[#23333b] rounded-[104px] transition-all duration-300 px-[27px] py-4 border border-[#23333b] hover:text-[#23333b] hover:bg-white"
                    >
                        お問い合わせ
                    </a>
                ...
                <div className="header__hamberger sp max-[1199px]:absolute max-[1199px]:top-2 max-[1199px]:right-4 z-[999999999999] min-[1200px]:hidden">
                    <button
                        aria-label="メニューを開閉"
                        onClick={() => setOpen((o) => !o)}
                        className={`openbtn1 relative bg-white w-[50px] h-[50px] rounded-[5px] ${open ? "active" : ""}`}
                    >
                        {[0, 1, 2].map((i) => (
                            <span
                                key={i}
                                className={`absolute left-[14px] h-[3px] rounded-[2px] bg-[#23333b] transition-all duration-300 ${i === 0
                                        ? open
            ...
            {/* モバイル: 展開メニュー */}
            <div
                className={`header__top-menu vertical-menu max-[1199px]:block fixed left-0 bg-white w-full transition-[max-height] duration-300 overflow-hidden ${open ? "max-h-[600px]" : "max-h-0"
            ...
            >
                {MENU_ITEMS.map((item, idx) => (
                    <a
                        ...
                        target={item.external ? "_blank" : undefined}
                        rel={item.external ? "noopener noreferrer" : undefined}
                        className={`block text-[16px] font-bold py-5 px-4 text-center ${idx === MENU_ITEMS.length - 1 ? "pb-10" : ""
                            }`}
                    >
                        {item.ruby ? (
                            <ruby>
                                {item.ruby.base} <rt className="block text-[40%] -mb-2 tracking-[5px]">{item.ruby.rt}</rt>
                            </ruby>
                        ) : (
                            item.label
                        )}
